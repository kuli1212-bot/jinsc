        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row items-center">
            ...
            <div className="lg:w-1/2 relative">
              <img src="https://duk.tw/FkCMrZ.png" alt="Factory" className="w-full h-auto rounded-[48px] shadow-2xl" />
            </div>
          </div>
        </div>
      </section>
    ...
    if (path.includes('/philosophy')) {
      return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <h1 className="text-4xl font-black text-slate-900 mb-8 border-b-4 border-blue-600 inline-block">{text.philosophy_title}</h1>
          <p className="text-lg text-slate-600 leading-relaxed font-medium mb-12">{text.philosophy_content}</p>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {text.philosophy_list.map((item: string, i: number) => (
              <div key={i} className="p-8 bg-white rounded-3xl border border-slate-100 font-black text-blue-600 text-center shadow-sm text-xl flex items-center justify-center">
                {item}
              </div>
            ))}
          </div>
      ...
      return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <h1 className="text-4xl font-black text-slate-900 mb-8 border-b-4 border-blue-600 inline-block">{lang === 'zh' ? '精密設備' : 'Machinery'}</h1>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {EQUIPMENT_LIST.map((eq, i) => (
              <div key={i} className="bg-white border border-slate-100 rounded-[40px] overflow-hidden shadow-sm hover:shadow-lg transition-all">
                <div className="aspect-square bg-slate-50 p-6">
                  <img src={eq.image} alt={eq.name} className="w-full h-full object-contain" />
                </div>
                <div className="p-8">
                  <h4 className="text-xl font-black text-slate-900 mb-1">{eq.name}</h4>
                  <p className="text-blue-600 text-xs font-bold uppercase mb-4">{eq.brand}</p>
      ...
      return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <h1 className="text-4xl font-black text-slate-900 mb-8 border-b-4 border-blue-600 inline-block">{text.processing_title}</h1>
          <div className="flex flex-col lg:flex-row gap-12 items-start">
            <div className="lg:w-1/2">
              <p className="text-lg text-slate-600 leading-relaxed font-medium mb-8">{text.processing_content}</p>
              <div className="grid grid-cols-1 gap-4">
                {text.processing_items.map((item: string, i: number) => (
                  <div key={i} className="flex items-center space-x-4 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                    <div className="w-8 h-8 bg-blue-600 rounded-full flex items-center justify-center text-white font-bold shrink-0">{i+1}</div>
            ...
            <div className="lg:w-1/2">
              <img src={text.processing_image} alt="Processing" className="w-full rounded-[48px] shadow-2xl" />
            </div>
          </div>
        </div>
      );
      ...
      return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <h1 className="text-4xl font-black text-slate-900 mb-8 border-b-4 border-blue-600 inline-block">{text.iso_title}</h1>
          <div className="bg-slate-900 p-12 rounded-[48px] text-white">
            <h2 className="text-3xl font-black text-blue-400 mb-6">{text.iso_subtitle}</h2>
            <p className="text-lg text-slate-300 leading-relaxed mb-8">{text.iso_content}</p>
            <div className="inline-block p-6 bg-blue-600 rounded-3xl">
              <p className="text-2xl font-black italic">{text.iso_policy}</p>
            </div>
          </div>
        </div>
      );
      ...
      return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          <h1 className="text-4xl font-black text-slate-900 mb-8 border-b-4 border-blue-600 inline-block">{text.history_title}</h1>
          <div className="flex flex-col lg:flex-row gap-12 items-start mb-16">
            <div className="lg:w-1/2">
              <p className="text-lg text-slate-600 leading-relaxed font-medium">{text.history_content}</p>
            </div>
            <div className="lg:w-1/2">
              <img src={text.history_image} alt="History" className="w-full rounded-[48px] shadow-2xl" />
            </div>
          </div>
        </div>
      );
...
const DownloadPage = ({ lang }: { lang: 'zh' | 'en' }) => {
  ...
  return (
    <div className="pt-40 pb-24 bg-white min-h-screen relative">
      {selectedImage && (
        <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-sm flex items-center justify-center p-4 cursor-zoom-out animate-in fade-in duration-300" onClick={() => setSelectedImage(null)}>
          <div className="relative max-w-5xl max-h-[90vh] w-full flex items-center justify-center">
            <img src={selectedImage} className="max-w-full max-h-full object-contain rounded-2xl shadow-2xl bg-white" alt="Enlarged Catalog" />
            <button className="absolute -top-12 right-0 text-white hover:text-blue-400 transition-colors" onClick={() => setSelectedImage(null)}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h1 className="text-5xl font-black text-slate-900 mb-6 tracking-tighter">{labels.download}</h1>
          <p className="text-slate-500 text-lg font-medium">{labels.download_hint}</p>
        </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {DOWNLOAD_RESOURCES.filter(res => res.category === cat.id).map((res: any) => (
                <div key={res.id} className="group flex flex-col space-y-3">
                  <div className="aspect-[3/4] bg-slate-50 rounded-[32px] overflow-hidden border border-slate-100 shadow-sm cursor-zoom-in relative group" onClick={() => setSelectedImage(res.image)}>
                    <img src={res.image} alt={lang === 'zh' ? res.title_zh : res.title_en} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                  ...
                  <div className="flex items-center justify-between px-2">
                    ...
                    <a href={res.image} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 flex items-center space-x-1">
...
const ContactPage = ({ lang }: { lang: 'zh' | 'en' }) => {
  ...
  return (
    <div className="pt-40 pb-24 bg-white min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h1 className="text-5xl font-black text-slate-900 mb-6 tracking-tighter">{text.title}</h1>
          <p className="text-slate-500 text-lg font-medium">{text.subtitle}</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          <div className="space-y-8">
            <div className="bg-slate-50 p-8 rounded-[40px] border border-slate-100 shadow-sm">
              <div className="space-y-8">
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    <h4 className="text-xs font-black text-blue-600 uppercase tracking-widest mb-1">{text.address_label}</h4>
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">{text.phone_label}</h4>
                    <p className="text-2xl font-black text-slate-900">{CONTACT_INFO.phone}</p>
                  </div>
                </div>

                <div className="flex items-start space-x-4">
                  <div className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-900 shrink-0 border border-slate-200 shadow-sm">
                  ...
                  <div>
                    <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">{text.fax_label}</h4>
                    <p className="text-xl font-bold text-slate-700">{CONTACT_INFO.fax}</p>
                  </div>
                </div>

                <div className="flex items-start space-x-4">
                  <div className="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-blue-600 shrink-0 border border-blue-100 shadow-sm">
                    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                  </div>
                  <div>
                    <h4 className="text-xs font-black text-blue-600 uppercase tracking-widest mb-1">{text.email_label}</h4>
                    <p className="text-lg font-black text-blue-600 underline underline-offset-4">{CONTACT_INFO.email}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-900 p-8 rounded-[40px] text-white flex items-center justify-between">
              <div>
                <p className="text-blue-400 text-xs font-black uppercase tracking-widest mb-1">Business Hours</p>
                <p className="font-bold">Mon - Fri: 08:30 - 17:30</p>
              </div>
              <a href={`mailto:${CONTACT_INFO.email}`} className="px-6 py-3 bg-blue-600 hover:bg-blue-500 transition-all rounded-2xl font-black text-sm shadow-xl">{CONTACT_PAGE_TEXT[lang].consultation}</a>
            </div>
          </div>

          <div className="h-[500px] lg:h-full min-h-[600px] rounded-[48px] overflow-hidden shadow-2xl border border-slate-100 relative group bg-slate-100">
...
const ProductsPage = ({ categoryId, lang }: { categoryId?: string, lang: 'zh' | 'en' }) => {
    ...
    return (
      <div className="pt-24 pb-20 bg-white min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16 text-center">
          <h1 className="text-4xl lg:text-5xl font-black text-slate-900 mb-6 tracking-tighter">{currentCategory?.label}</h1>
          <div className="flex justify-center flex-wrap gap-3 mt-8">
            {PRODUCT_CATEGORIES[lang].map((cat: any) => (
              <Link key={cat.id} to={cat.path} className={`px-8 py-3 rounded-2xl text-sm font-bold border transition-all ${categoryId === cat.id ? 'bg-slate-900 text-white border-slate-900 shadow-lg' : 'border-slate-200 text-slate-500 hover:border-slate-900 hover:text-slate-900'}`}>{cat.label}</Link>
            ))}
          </div>
        </div>
        
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-slate-50 rounded-[48px] overflow-hidden border border-slate-100 p-8 lg:p-16">
            <div className="flex flex-col lg:flex-row gap-12 items-center">
              <div className="lg:w-1/2">
                <img src={currentCategory?.featuredImage} alt={currentCategory?.label} className="w-full h-auto rounded-[32px] shadow-2xl bg-white" />
              </div>
              <div className="lg:w-1/2">
                <p className="text-lg text-slate-600 font-medium leading-relaxed mb-8 italic">{currentCategory?.description}</p>
                <h4 className="text-xl font-black text-slate-900 mb-6">{NAV_LABELS[lang].list_prefix}</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {currentCategory?.items?.map((item: string, i: number) => (
                    <div key={i} className="flex items-center space-x-3 p-4 bg-white rounded-2xl border border-slate-100 shadow-sm">
                      <div className="w-6 h-6 bg-blue-600 rounded-full flex items-center justify-center text-white text-xs font-black shrink-0">{i+1}</div>
  ...
  return (
    <div className="pt-24 pb-20 bg-white min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16 text-center">
        <h1 className="text-4xl lg:text-5xl font-black text-slate-900 mb-6 tracking-tighter">{currentCategory ? currentCategory.label : 'Products'}</h1>
        <div className="flex justify-center flex-wrap gap-3 mt-8">
          {PRODUCT_CATEGORIES[lang].map((cat: any) => (
            <Link key={cat.id} to={cat.path} className={`px-8 py-3 rounded-2xl text-sm font-bold border transition-all ${categoryId === cat.id ? 'bg-slate-900 text-white border-slate-900 shadow-lg' : 'border-slate-200 text-slate-500 hover:border-slate-900 hover:text-slate-900'}`}>{cat.label}</Link>
          ))}
